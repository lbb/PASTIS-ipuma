...
void pw_aln_batch(
  ...
  parops->info(
      "Starting batch pairwise alignments.\n  "
      "#elems prior to pre-pruning: " +
      to_string(nnzs));
  ...
  // IPUMA aligner: one seq cannot be greater than 1024
...
template void pw_aln_batch<CommonKmerLoc>(std::shared_ptr<DistFastaData>,
                                          combblas::SpParMat<uint64_t, CommonKmerLoc,
                                                             combblas::SpDCCols<uint64_t, CommonKmerLoc>> &,
                                          PWAlign &, params_t &, int, int);

template void pw_aln_batch<CommonKmerLight>(std::shared_ptr<DistFastaData>,
