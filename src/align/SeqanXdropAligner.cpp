...
void SeqanXdropAligner::construct_seqs(std::shared_ptr<DistFastaData> dfd) {
  parops->logger->log("AAA");
...
#pragma omp for
      for (uint64_t i = 0; i < nbr_seqs_count; ++i) {
        ...
        cur_seqs[seq_beg + i] = std::move(seqan::Peptide(buff + start_offset, len));
      }
    }
...
#if PASTIS_DBG_LVL > 0
  uint64_t tot_sz = 0;
  for (auto &el : rseqs_) tot_sz += seqan::capacity(el) * sizeof(char);
  for (auto &el : cseqs_) tot_sz += seqan::capacity(el) * sizeof(char);
...
void SeqanXdropAligner::construct_seqs_bl(std::shared_ptr<DistFastaData> dfd) {
  parops->logger->log("BBB");
  string s_tmp;
  ...
  parops->logger->log("#rseqs " + to_string(rseq_cnt) + " #cseqs " + to_string(cseq_cnt));
...
void SeqanXdropAligner::aln_batch( std::tuple<uint64_t, uint64_t, CommonKmerLoc *> *mattuples, uint64_t beg, uint64_t end, uint64_t bl_roffset, uint64_t bl_coffset, const params_t &params) {
  parops->logger->log("CCC");
...
#pragma omp for
  for (uint64_t i = beg; i < end; ++i) {
    ...
    assert(lr + bl_roffset < rseqs_.size() && lc_bl_coffset < cseqs_.size());

    seqsr[i - beg] = std::move(seqan::Gaps<seqan::Peptide>(rseqs_[lr + bl_roffset]));
    seqsc[i - beg] = std::move(seqan::Gaps<seqan::Peptide>(cseqs_[lc + bl_coffset]));
  }
  ...
  vector<size_t> slenr(npairs);
  for (size_t i = 0; i < npairs; i++) {
    auto &el = seqsr[i];
    slenr[i] = seqan::capacity(el);
    tot_sz += seqan::capacity(el) * sizeof(char);
  }

  ofstream slr_fd;
  slr_fd.open (to_string(rxcount) + "slenr.txt");
  for (size_t i = 0; i < npairs; i++) {
    slr_fd << slenr[i] << "\n";
  }
  slr_fd.close();
  ...
  vector<size_t> slenc(npairs);
  for (size_t i = 0; i < npairs; i++) {
    auto &el = seqsc[i];
    slenc[i] = seqan::capacity(el);
    tot_sz += seqan::capacity(el) * sizeof(char);
  }

  ofstream slc_fd;
  slc_fd.open (to_string(rxcount) + "slenc.txt");
  for (size_t i = 0; i < npairs; i++) {
    slc_fd << slenc[i] << "\n";
  }
  slc_fd.close();
  ...
  parops->logger->log("approximate memory allocated for alignment seqs " + gb_str(tot_sz));
// parops->bytes_alloc += tot_sz;
// parops->logger->log("approximate memory in usage " +
//                                      gb_str(parops->bytes_alloc));
#endif
...
#pragma omp parallel for
    for (uint64_t i = 0; i < npairs; ++i) {
      ...
      TSeed seed(l_col_seed_start_offset, l_row_seed_start_offset, seed_len_);
      extendSeed(seed, seqan::source(seqsc[i]), seqan::source(seqsr[i]), seqan::EXTEND_BOTH, blosum62_, xdrop_, seqan::GappedXDrop());
      assignSource(seqsc_ex[i], infix(seqan::source(seqsc[i]), beginPositionH(seed), endPositionH(seed)));
      assignSource(seqsr_ex[i], infix(seqan::source(seqsr[i]), beginPositionV(seed), endPositionV(seed)));
    }
...
#pragma omp parallel for
      for (uint64_t i = 0; i < npairs; ++i) {
        computeAlignmentStats(stats[i], seqsc_ex[i], seqsr_ex[i], blosum62_);
      }
    } else {
#pragma omp parallel for
      for (uint64_t i = 0; i < npairs; ++i) {
        seqan::AlignmentStats tmp;
        computeAlignmentStats(tmp, seqsc_ex[i], seqsr_ex[i], blosum62_);
...
#pragma omp for
    for (uint64_t i = 0; i < npairs; ++i) {
      ...
      double alen_minus_gapopens = tmp.alignmentLength - tmp.numGapOpens;
      ...
      if (std::max((alen_minus_gapopens / len_seqc),
        ...
        CommonKmerLoc *ckl = std::get<2>(mattuples[beg + i]);
        ckl->score_aln = static_cast<float>(tmp.alignmentIdentity) / 100.0f;
      }
    }
  }
